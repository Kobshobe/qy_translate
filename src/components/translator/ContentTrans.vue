<template>
  <ThemeElement>
    <div v-if="(baseHook.mode === 'pdf' || baseHook.C.isTreadWord) &&
      text.replace(/\s/g, '') !== ''
      " class="
        tap-to-translate
        to-translate-icon
      " @click="translate" :style="toTranslateStyle">
      <svg width="23px" height="23px" style="background-color: transparent;" viewBox="0 0 434 434" version="1.1" xmlns="http://www.w3.org/2000/svg"
        xmlns:xlink="http://www.w3.org/1999/xlink">
        <title>logo</title>
        <g id="logo" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
          <rect fill="#FFFFFF" opacity="0" x="0" y="0" width="434" height="434"></rect>
          <rect id="矩形备份" stroke="#4C8BF5" stroke-width="21" fill="#FFFFFF" x="209.5" y="46.5" width="201" height="349"
            rx="33"></rect>
          <path
            ...
            id="文" fill="#4C8BF5" fill-rule="nonzero"></path>
          <path
          ...
          <path
            ...
            id="En备份" fill="#FFFFFF" fill-rule="nonzero"></path>
        </g>
      </svg>
    </div>

    <div v-if="baseHook.findStatus === 'popLoading'" ref="transLoadingDOM" class="content-trans-loading"
      :style="toTranslateStyle">
      <Loading />
    </div>

    <div v-if="baseHook.isHold ||
      (baseHook.status !== 'none' && baseHook.findStatus !== 'popLoading') ||
      (baseHook.toast.show || baseHook.dialog.show)
      " ref="resultDOM" class="translator-result-box" :style="resultStyle">
      <Translator />
    </div>
  </ThemeElement>
</template>
    ...
    if (resultDOM.value) {
      ...
      e.composedPath().some((elm: any) => {
        if (typeof elm.className === "string") {
          if (elm.className.indexOf("xx-select__popper") !== -1) {
            out = false;
            return true;
          }
        }
