...
class YoudaoTranslator {
     ...
     * @returns {Promise} then(result) used to return request result. catch(error) used to catch error
     */
    translate(text:string, from = "AUTO") {
        let reTryCount = 0;
        ...
        let translateOneTime:any = async () => {
            ...
            return fetch(
                this.HOST + "/translate", // + "?" + "from=" + fromCode + "&to=" + toCode, _o会err:50
                {
                method: "post", 
                headers: this.HEADERS,
                body: this.getQueryStr(text), // includes sign
                }
            ).then(async (res) => {
                //console.log("HTTP status:", result.status);
                // console.log("HTTP statusText:", result.statusText);
                // console.log("result.data\n", result.data);
                let transResult:ITransResult;
                if (res.status === 200) {
                    const resultText = await res.text()
                    const body = JSON.parse(resultText)
                    const result = body.translateResult[0][0]
                    transResult = {
                        text: result.tgt,
                        resultFrom: 'en',
                        resultTo: 'zh_CN',
                ...
                // }
                return Promise.resolve(this.parseResult(transResult));
            });
        };
